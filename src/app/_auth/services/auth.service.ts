...
export class AuthService {
  ...
  async SaveLocation(busNo,Lang,lat): Promise<any>  {
    const locationData  = {
      'busNo' : busNo,
      'Lang' : Lang,
      'lat' : lat,
    };
    const data  = await this.http.post(environment['apiBaseUrl'] + '/api/savelocation' , locationData).toPromise();

    // this part only gets executed when the promise is resolved
    if (data['busNo'] && data['Lang']) {

        this.setLocationData(data);        //this.isLoggedIn.next(true); // how do I unit test this?
  ...
  async GetLocation(busNo): Promise<any>  {
    const locationData  = {
      'busNo' : busNo,
      
    };
    const data  = await this.http.get(environment['apiBaseUrl'] + '/api/savelocation' ).toPromise();
